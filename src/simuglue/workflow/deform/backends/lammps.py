...
class LAMMPSBackend(Backend):
    ...
    def read_data(self, cfg: Config):
        """Read the initial structure from a LAMMPS data file."""
    ...
    def prepare_case(self, case_dir: Path, atoms, cfg: Config) -> None:
        ...
        write_lammps_data(
            case_dir / "str.data",
            atoms,
            atom_style=cfg.lammps.get("atom_style", "full"),
        ...
        # Get the component index (1, 2, or 3) passed from the workflow
        comp = getattr(cfg, "active_component", 1)

        if comp == 1:
            relax_axis = "y 0.0" # Pulling X, relax Y
        elif comp == 2:
            relax_axis = "x 0.0" # Pulling Y, relax X
        else:
         relax_axis = "x 0.0 y 0.0"

        tpl = tpl.replace("${relax_dir}", relax_axis)

        # FIX: Corrected the mapping for yz
        print_line = (
    ...
    def run_case(self, case_dir: Path, cfg: Config) -> None:
        ...
        try:
            with log.open("w") as f:
                ...
                f.flush()
                subprocess.run(
                    cmd, cwd=case_dir, check=True, stdout=f,
            ...
            if (case_dir / "thermo.json").is_file():
                ...
                finalize_failure(case_dir, "thermo.json missing.")
    ...
    def parse_case(self, case_dir: Path, cfg: Config) -> RelaxResult:
        if not is_done(case_dir):
            raise ValueError("Case not done.")

        data = json.loads((case_dir / "thermo.json").read_text(encoding="utf-8"))
        lammps_units = cfg.lammps.get("units", "metal")

        # Check for failure
        check_vals = [data["pxx"], data["pyy"], data["pzz"], data["pe"]]
        if not np.isfinite(check_vals).all():
            print(f"Warning: Failure detected in {case_dir.name}")
            return RelaxResult(energy=float('nan'), stress=np.zeros((3,3)), cell=np.eye(3))
